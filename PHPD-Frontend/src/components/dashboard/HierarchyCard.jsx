...
export function HierarchyCard({ title, overallProgress, onClick, className, color }) {
  ...
  return (
    React.createElement(Card, { 
      ...
      , React.createElement('div', { className: cn("absolute inset-0 z-0", colors.bg)})
      
      /* Subtle light blow effect */
      , React.createElement('div', { className: cn("absolute top-0 right-0 w-32 h-32 rounded-full blur-2xl -mr-16 -mt-16 opacity-20 group-hover:opacity-35 transition-opacity z-0 bg-white/50")})

      , React.createElement(CardContent, { className: "relative z-10 p-4" }
        , React.createElement('div', { className: "flex flex-col gap-3"}
          /* Title row (simple, no inner panel) */
          , React.createElement('div', { className: "flex items-start justify-between gap-3"}
            , React.createElement('div', { className: "min-w-0"}
              , React.createElement('h3', { className: "truncate text-[13px] sm:text-sm font-bold font-heading text-slate-800 dark:text-white"}
                , title
              )
              , React.createElement('div', { className: "mt-1 inline-flex items-center rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-widest text-slate-700/80 dark:text-slate-200 border border-white/40 bg-white/35"}
                , status
              )
            )
            , React.createElement('div', { className: cn("flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-border/40 shadow-sm bg-white/50", colors.iconBg)}
              , React.createElement(TrendingUp, { className: cn("h-4 w-4", colors.text)} )
            )
          )
          ...
          , React.createElement('div', { className: "flex items-end justify-between"}
            , React.createElement('div', { className: "flex items-baseline gap-1"}
              , React.createElement('span', { className: cn("text-[22px] sm:text-[24px] font-extrabold font-heading tabular-nums tracking-tight leading-none", colors.text)}
                , progressLabel
              )
              , React.createElement('span', { className: cn("text-[11px] font-bold opacity-80", colors.text)}, "%")
            )
            , React.createElement(ArrowRight, { className: cn("h-5 w-5 text-slate-600/60 group-hover:translate-x-1 group-hover:text-slate-900 transition-all")} )
          )

          /* Progress bar (slimmer, cleaner like your 3rd example) */
          , React.createElement('div', { className: "relative h-2 w-full overflow-hidden rounded-full bg-white/50 border border-white/40 shadow-inner"}
            , React.createElement('div', {
