  ...
  return (
    React.createElement(Card, { className: "shadow-lg border-border/50 border-2 transition-colors hover:border-primary/60"    }
      , React.createElement(CardHeader, { className: "pb-4"}
        , React.createElement(CardTitle, { className: "font-heading text-xl font-bold"  }, "Phase Breakdown" )
        , React.createElement(CardDescription, { className: "text-sm"}, "Installation progress by phase"   )
      )
      , React.createElement(CardContent, {}
        , React.createElement('div', { className: "w-full", style: { height: isMobile ? '240px' : isTablet ? '280px' : '300px' }}
          , React.createElement(ResponsiveContainer, { width: "100%", height: "100%"}
            ...
            , React.createElement(Bar, { dataKey: "percentage", radius: [4, 4, 0, 0]}
              , chartData.map((entry, index) => (
                React.createElement(Cell, { key: `cell-${index}`, fill: getColor(entry.phase)} )
              ))
            )
          )
        )
